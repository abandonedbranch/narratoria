# Data Model: Dart/Flutter Implementation

**Purpose**: Defines Dart class implementations for Spec 001/002 contracts
**Part of**: [Spec 003: Dart/Flutter Implementation](spec.md)
**Status**: Draft
**Implements**: [Spec 002 Plan Execution Semantics](../002-plan-generation-skills/spec.md)

## Overview

This document defines the Dart class implementations for Narratoria's plan execution system. These classes implement the architectural contracts defined in Spec 002 using Dart-specific data structures and algorithms. This data model provides:
...
Narrator AI generates this structure for the executor. Implements Spec 002 Plan JSON Schema Plan JSON Schema.

```dart
/// Plan JSON document generated by narrator AI.
/// Implements Spec 002 Plan JSON Schema extended schema.
class PlanJson {
...
/// Implements Spec 002 Plan JSON Schema ToolInvocation schema.
...
/// Implements Spec 002 Plan Execution Semantics.5 retry requirements.
class RetryPolicy {
  final int maxRetries;                             // Default: 3
  final int backoffMs;                              // Exponential backoff base
  
  /// Calculates backoff delay per Spec 002 reference formula.
...
During execution, the executor maintains state per Spec 002 Plan Execution Semantics requirements.

```dart
/// Implements plan execution semantics from Spec 002 Plan Execution Semantics.
/// 
/// Protocol requirements satisfied:
/// - MUST detect circular dependencies (Spec 002 Plan Execution Semantics.1)
/// - MUST execute tools in dependency-respecting order (Spec 002 Plan Execution Semantics.2)
/// - MUST respect `required` flag for failure propagation (Spec 002 Plan Execution Semantics.6)
/// - MUST retry up to maxRetries with exponential backoff (Spec 002 Plan Execution Semantics.5)
///
...
class PlanExecutionContext {
  ...
  /// 
  /// Protocol requirement: Spec 002 Plan Execution Semantics.2
  ...
  /// 
  /// Protocol requirement: Spec 002 Plan Execution Semantics.1
  bool detectsCycle() { ... }
...
/// Implements Spec 002 Execution Result schema ToolResult structure.
class ToolResult {
  final String toolId;
