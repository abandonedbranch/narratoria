...
- **FR-011**: System MUST update the per-session StoryState (summary/characters/inventory) after each processed TextChunk by writing the updated state into PipelineChunkMetadata.Annotations using the key narratoria.story_state_json.
  - **Within a single pipeline run (single stream execution)**: Transforms MUST carry forward the latest StoryState between chunks and write it into each output chunk’s annotations, so later chunks in the same run can observe the updated state even if the upstream source repeats stale/initial metadata.
  - **Across turns/runs**: Continuity is achieved by the caller/orchestrator injecting the most recent narratoria.story_state_json value onto subsequent incoming chunks; transforms MUST treat a missing value as an empty/default state and MUST NOT implement durable storage in this feature.
