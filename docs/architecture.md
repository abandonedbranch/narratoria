...
2. Chunk by **field context**: Each major field becomes a searchable unit
   ...
   - **Player character profile**: Generated at campaign start from fresh character template (freeform description). LLM creates structured JSON based on campaign world and constraints, then profile is processed identically to NPC profiles with the same chunking strategy (separate chunks for `personality`, `background`, `goals`, `speech_patterns`). Distinguished by `is_player_character: true` flag in metadata.
...
**Player Character Realization and Ingestion**: When a player selects a fresh character for a campaign (via campaign start), the system performs two additional steps before standard ingestion:

1. **Character Realization** (2-5 seconds): LLM (Phi-4/Phi-4-mini) generates structured character.json from the fresh character's freeform description, tailored to the campaign's world, tone, and constraints (allowed races/classes). Generated profile includes name, archetype, personality, background, goals, speech_patterns, and starting stats.

2. **Realization Ingestion**: Generated character.json is chunked and embedded alongside campaign NPCs using the same pipeline. The character becomes part of the campaign's semantic knowledge graph, enabling semantic retrieval during gameplay without injecting the full profile into LLM context. The `character_id` field in session state references the original fresh character, while chunks stored in ObjectBox contain the realized character data.
