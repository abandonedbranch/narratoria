...
**Core Principle**: *Player characters are swept into the campaign world through the same ingestion pipeline as NPCs, becoming part of the semantic fabric of the story.*

Player Character Profiles enable players to create persistent character personas that travel across campaigns. Unlike traditional approaches where character data is injected into AI context as raw JSON, Narratoria **ingests player characters into ObjectBox** alongside campaign data during campaign load, treating them as semantically-searchable entities within the narrative knowledge graph.

**Key Design Tenets:**

1. **Characters as Semantic Entities**: Player characters undergo the same chunking, embedding, and indexing as NPCs, enabling the Narrator AI to retrieve character details through semantic queries rather than context injection.
2. **Memory Footprint Optimization**: By storing characters in ObjectBox rather than injecting full profiles into LLM context, the system minimizes memory usage while maintaining narrative coherence.
3. **Campaign Authority**: The campaign's world, constraints, and narrative always override player-provided character data where conflicts exist.
4. **LLM-Assisted Creation**: Characters are generated by the in-process language model from freeform text descriptions, then presented for player review and confirmation.

##### Storage and Ingestion Model

Player characters exist in two states:

**1. Persistent Storage (Pre-Campaign)**

Characters are stored in the application's user data directory as JSON files:
...
characters/
├── knight_eredin.json
├── knight_eredin_portrait.webp
├── dragon_emberwing.json
└── dragon_emberwing_portrait.webp
```

**2. Ingested State (During Campaign)**

When a campaign loads with a selected player character:

1. **Character JSON is ingested** alongside campaign NPCs, lore, and world data
2. **Marked as player character** with `is_player_character: true` metadata flag
3. **Chunked and embedded** using the same paragraph-based semantic embedding as NPC profiles
4. **Indexed in ObjectBox** with relationships to campaign entities (locations, NPCs, plot beats)
5. **Retrieved via semantic search** when the Narrator AI needs character details

This approach ensures the player character **becomes part of the campaign's knowledge graph** rather than being externally injected.

##### Character Profile Schema

Player character JSON follows the same structure as NPC profiles (Section 6.2.4):
...
```json
{
  ...
  "last_used": "2026-02-12T09:15:00Z",
  
  "archetype": {
    "race": "Human",
    "class": "Knight",
    "subclass": "Oath of Protection"
  },
  
  "personality": {
    "traits": ["honorable", "protective", "stoic", "duty-bound"],
    "flaws": ["rigid", "guilt-ridden", "distrusts magic"],
    "virtues": ["courage", "loyalty", "humility"]
  },
  
  "speech_patterns": {
    "style": "Formal, measured tones",
    "examples": [
      "By my oath, I shall stand between the innocent and harm.",
      "Duty demands I act, though wisdom might counsel patience."
    ]
  },
  
  "backstory": "A knight who failed to protect their liege in a prior campaign. Now seeks redemption through unwavering service to the innocent.",
  
  "goals": [
    {"goal": "Protect the innocent", "priority": "high"},
    {"goal": "Seek redemption for past failures", "priority": "high"}
  ],
  ...
  "portrait": "characters/knight_eredin_portrait.webp",
...
**Required Fields:**
- `id`, `name`, `role`, `is_player_character`, `version`, `created_at`

**Optional Fields:**
- `archetype`, `personality`, `speech_patterns`, `backstory`, `goals`, `starting_stats`, `portrait`, `campaign_history`, `last_used`

##### LLM-Assisted Character Creation

Characters are created through an AI-assisted workflow that reduces authoring friction:

**Creation Flow:**

1. **Player Provides Freeform Description**
   - UI: Single multiline text input field
   - Example input: "A gruff, battle-scarred knight who secretly loves poetry. Failed to save his king and now seeks redemption."
   - No structured form fields required

2. **LLM Generates Structured Character JSON**
   - In-process model (Phi-4 or Phi-4-mini) generates complete character profile
   - Prompt includes campaign constraints (allowed_races, allowed_classes) to ensure compatibility
   - Output: Fully-formed JSON matching character profile schema

3. **Player Reviews Generated Character**
   - UI displays generated character data in readonly format:
     - Name, archetype, personality traits/flaws/virtues
     - Backstory (expanded from input)
     - Speech pattern examples (generated to match personality)
     - Starting stats (derived from archetype)
   - Action buttons: Regenerate, Edit Description, Confirm, Cancel

4. **Player Uploads Portrait**
   - User-provided image (PNG/JPEG/WebP, max 5MB)
   - Resized to 512×512px
   - **No AI-generated portraits** (in-process models cannot generate images)

5. **Character Saved to Storage**
   - JSON written to `<user_data>/characters/{id}.json`
   - Portrait saved to `<user_data>/characters/{id}_portrait.{ext}`

**LLM Generation Prompt Template:**

```
Generate a character profile in JSON format from this description:
...
Output a complete character JSON with:
- name (creative but fitting description)
- archetype (race, class, subclass matching constraints)
- personality (traits, flaws, virtues arrays)
- speech_patterns (style description + 2-3 example lines)
- backstory (expanded narrative from description)
- goals (2-4 character goals with priority)
- starting_stats (appropriate values for archetype)

Ensure all fields are complete and internally consistent.
```

**Performance:**
- Generation time: 2-5 seconds (typical for Phi-4-mini)
- User can regenerate multiple times until satisfied

##### Character Editing

Editing follows the same LLM-assisted pattern:

1. **Player Provides Edit Description**
   - UI: Empty multiline text input
   - Example: "Make him less gruff, add a secret fear of magic"
   
2. **LLM Generates Updated Character JSON**
   - Receives existing character JSON + edit description
   - Produces modified JSON with requested changes
   - Preserves unchanged fields

3. **Player Reviews Changes**
   - UI displays diff view showing before/after changes
   - Action buttons: Regenerate Edit, Confirm, Cancel

4. **Updated Character Saved**
   - Existing JSON file overwritten
   - `last_used` timestamp **not** updated (only updated when used in campaign)

##### Portrait Management

**User-Provided Portraits Only:**

Narratoria's in-process models (Phi-4, sentence-transformers) cannot generate images. Players must upload their own portraits oder use placeholder icons.

```
characters/
├── knight_eredin.json
├── knight_eredin_portrait.webp      # User-uploaded
├── dragon_emberwing.json
└── dragon_emberwing_portrait.webp   # User-uploaded
```

**Portrait Handling:**
- Uploaded images resized to 512×512px (preserves aspect ratio with letterboxing if needed)
- Supported formats: PNG, JPEG, WebP
- Max file size: 5MB
- If no portrait provided: Generic placeholder icon displayed in UI

##### Campaign Integration and Ingestion

When a player starts a campaign with a selected character:

**Phase 1: Pre-Ingestion Filtering**

```
1. Load player character JSON from storage
2. Check campaign constraints:
   IF character.archetype.race NOT IN campaign.allowed_races:
     → Show warning: "This character's race will be adapted to {campaign_default_race}"
   IF character.archetype.class NOT IN campaign.allowed_classes:
     → Show warning: "This character's class will be adapted to {campaign_default_class}"
3. Player confirms selection with awareness of adaptations
```

**Phase 2: Character Ingestion Pipeline**

Player character JSON is processed identically to NPC profiles during campaign ingestion:
...
2. **Chunking**
   ...
   - Backstory chunk: Full backstory text (paragraph-based splitting if >512 tokens)
   - Goals chunk: `"Character goals: {goals list with priorities}"`
   - Speech patterns chunk: `"Speech style: {style}. Examples: {examples}"`

3. **Semantic Embedding**
   - Each chunk embedded with sentence-transformers (384-dim vectors)
   - Stored with references to parent entity (`{character.id}`)
...
4. **ObjectBox Storage**
   ...
   CampaignChunk {
     ...
     metadata: {is_player_character: true}
   }
   ```

5. **Relationship Graph Construction**
   - Character linked to campaign entities via semantic similarity
   - Player character marked as central node in knowledge graph
   - Cross-references established with NPCs, locations, plot beats

**Phase 3: Runtime Retrieval**

During gameplay, the Narrator AI accesses player character data through semantic search:
...
Returns chunks:
  ...
  3. "Speech style: Formal, measured tones..." (similarity: 0.71)
```

The Narrator AI synthesizes retrieved chunks into narrative prose without having the entire character profile in context memory.

##### Session State Initialization

When campaign starts, player character data initializes session state:

```json
{
  "player": {
    "name": "Sir Eredin",
    "race": "Human",              // From archetype (or adapted)
    "class": "Knight",            // From archetype (or adapted)
    "stats": {                    // From starting_stats, clamped to campaign ranges
      "strength": 16,
      "dexterity": 10,
      "constitution": 14,
      "intelligence": 8,
    ...
    "character_id": "knight_eredin"  // Reference to ObjectBox entity
  }
}
```

The `character_id` enables skills to query the persistence layer for full character details when needed, without keeping all data in session state.

##### Character Management UI

The system provides a dedicated character management interface (see User Journey: Options → Characters):

**Features:**
- **Create Character**: Freeform text → LLM generates JSON → Review → Upload portrait → Confirm
- **Edit Character**: Select character → Freeform edit description → LLM generates updated JSON → Review changes → Confirm
- **Delete Character**: Remove JSON and portrait files
- **Character Gallery**: Grid view with portraits, names, archetypes, campaign counts
- **Campaign History**: View which campaigns each character has participated in

**Validation:**
- Name uniqueness checked before save
- JSON schema validated (required fields present, types correct)
- Portrait format and size validated (PNG/JPEG/WebP, <5MB)
- Campaign history updated automatically when character used in campaign
...
2. Chunk by **field context**: Each major field becomes a searchable unit
   ...
   - **Player character profile**: Processed identically to NPC profiles with the same chunking strategy (separate chunks for `personality`, `background`, `goals`, `speech_patterns`). Distinguished by `is_player_character: true` flag in metadata
...
**Player Character Ingestion**: When a player character is selected for a campaign (via session creation), the character's JSON profile (stored in `data/characters/`) is ingested into the campaign's ObjectBox database using this same pipeline. The character is chunked and embedded alongside campaign NPCs, enabling semantic retrieval during gameplay without injecting the full character JSON into LLM context. The `character_id` field in the session state references the ObjectBox entity for runtime lookups.
