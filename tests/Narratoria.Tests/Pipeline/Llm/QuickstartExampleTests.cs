...
{
    [TestMethod]
    public async Task Quickstart_ChainingExample_RunsEndToEndDeterministically()
    {
        const string rewritten = "Rewritten narration.";
        const string summary = "Rolling recap.";
        ...
        var service = new FakeTextGenerationService(req =>
        {
            ...
            {
                return new TextGenerationResponse { GeneratedText = summary };
            }
        ...
        var (outcome, collectedText, annotations) = await LlmPipelineHarness.RunAsync(
            [new TextChunk("Original narration.", metadata)],
            transforms);
