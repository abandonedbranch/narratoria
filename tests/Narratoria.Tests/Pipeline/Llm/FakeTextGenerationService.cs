...
public sealed class FakeTextGenerationService(Func<TextGenerationRequest, TextGenerationResponse> handler) : ITextGenerationService
{
    private readonly Func<TextGenerationRequest, TextGenerationResponse> _handler = handler ?? throw new ArgumentNullException(nameof(handler));
