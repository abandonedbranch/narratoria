...
- **Command routing**: Runtime commands now use slash prefixes (e.g., `/help`, `/export`, `/settings`). Slash commands render in a transient command host near the reply editor (not in scrollback or persistence). Workflow routing still uses leading `@` (e.g., `@system`), but invoking components is done via `/` commands.
