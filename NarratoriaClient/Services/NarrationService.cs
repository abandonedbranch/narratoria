...
public sealed class NarrationService : INarrationService
{
    ...
    public NarrationService(IAppDataService appData, IOpenAiChatService chatService, ILogger<NarrationService> logger, ILogBuffer logBuffer)
    {
    ...
    public async Task ProcessPlayerMessageAsync(string content, CancellationToken cancellationToken = default)
    {
        ...
        await _appData.AppendPlayerMessageAsync(trimmed, cancellationToken);
        NotifyStatus(NarrationStatus.Connecting, "Connecting to the narrator…");

        try
        {
            var narratorResponse = await GenerateNarrationAsync(cancellationToken).ConfigureAwait(false);
        ...
        catch (Exception ex)
        {
            ...
            NotifyStatus(NarrationStatus.Disconnected, "Narrator left the table. Retry to reconnect.");
    ...
    private async Task<string> GenerateNarrationAsync(CancellationToken cancellationToken)
    {
        ...
        NotifyStatus(NarrationStatus.Connecting, $"Connecting to {narratorModel}…");
        ...
        await foreach (var update in _chatService.StreamChatCompletionAsync(request, cancellationToken))
        {
            if (!streamStarted)
            {
                streamStarted = true;
                NotifyStatus(NarrationStatus.Writing, "Narrator is writing…");
            }
    ...
    private void NotifyStatus(NarrationStatus status, string message)
    {
        ...
        Log(LogLevel.Debug, "Narration status changed.", new Dictionary<string, object?>
        {
